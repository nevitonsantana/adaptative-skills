---
title: Skill reference
description: Read detailed public profiles generated from canonical Adaptive Skills contracts.
---

Use these profiles when the catalog summary is not enough to decide how a skill works, what it returns, or how to verify the result.

Each profile is generated from its canonical `skills/<name>/SKILL.md` file. The public page adds orientation and navigation, but it does not replace or independently rewrite the contract.

## Complete reference

| Skill | Category | Use it when |
|---|---|---|
| [`workflow`](https://nevitonsantana.github.io/adaptive-skills/skills/workflow/) | Engineering | Starting non-trivial work or returning from a handoff |
| [`feature-planning`](https://nevitonsantana.github.io/adaptive-skills/skills/feature-planning/) | Engineering | Turning a feature request into staged delivery |
| [`testing`](https://nevitonsantana.github.io/adaptive-skills/skills/testing/) | Engineering | Choosing proof before closing meaningful work |
| [`debugging`](https://nevitonsantana.github.io/adaptive-skills/skills/debugging/) | Engineering | Investigating a reproducible failure |
| [`api-design`](https://nevitonsantana.github.io/adaptive-skills/skills/api-design/) | Engineering | Defining a contract between components |
| [`refactoring`](https://nevitonsantana.github.io/adaptive-skills/skills/refactoring/) | Engineering | Improving structure without changing behavior |
| [`lean-implementation`](https://nevitonsantana.github.io/adaptive-skills/skills/lean-implementation/) | Engineering | Implementing a confirmed bounded plan |
| [`architecture-review`](https://nevitonsantana.github.io/adaptive-skills/skills/architecture-review/) | Engineering | Reviewing a consequential structural decision |
| [`code-style`](https://nevitonsantana.github.io/adaptive-skills/skills/code-style/) | Engineering | Improving consistency and readability |
| [`communication`](https://nevitonsantana.github.io/adaptive-skills/skills/communication/) | Engineering | Explaining a change, failure, option, or handoff |
| [`domain-language-alignment`](https://nevitonsantana.github.io/adaptive-skills/skills/domain-language-alignment/) | Engineering | Product, domain, docs, and code use different terms |
| [`ux-strategy`](https://nevitonsantana.github.io/adaptive-skills/skills/ux-strategy/) | Design | The interaction direction is still open |
| [`ux-provocation`](https://nevitonsantana.github.io/adaptive-skills/skills/ux-provocation/) | Design | A dominant UX hypothesis needs pressure testing |
| [`heuristic-audit`](https://nevitonsantana.github.io/adaptive-skills/skills/heuristic-audit/) | Design | An existing interface has usability risks |
| [`ux-writing`](https://nevitonsantana.github.io/adaptive-skills/skills/ux-writing/) | Design | Product language is unclear or unsafe |
| [`design-system-intelligence`](https://nevitonsantana.github.io/adaptive-skills/skills/design-system-intelligence/) | Design | Reviewing an artifact against declared design-system sources |
| [`documentation`](https://nevitonsantana.github.io/adaptive-skills/skills/documentation/) | Documentation | Creating, restructuring, or auditing onboarding, manuals, guides, and technical documentation |
| [`intent-clarification`](https://nevitonsantana.github.io/adaptive-skills/skills/intent-clarification/) | Planning | Outcome, success, or constraints are ambiguous |
| [`premortem`](https://nevitonsantana.github.io/adaptive-skills/skills/premortem/) | Planning | A consequential plan could fail in costly ways |
| [`task-chunking`](https://nevitonsantana.github.io/adaptive-skills/skills/task-chunking/) | Efficiency | Work is oversized or likely to sprawl |
| [`checkpoint-review`](https://nevitonsantana.github.io/adaptive-skills/skills/checkpoint-review/) | Efficiency | A long task needs a continue-or-stop decision |
| [`handoff-summary`](https://nevitonsantana.github.io/adaptive-skills/skills/handoff-summary/) | Efficiency | Work is crossing a person, agent, or session boundary |
| [`triad-check`](https://nevitonsantana.github.io/adaptive-skills/skills/triad-check/) | Cross-functional | A hard-to-reverse decision crosses product, design, and engineering |
| [`feature-value-governance`](https://nevitonsantana.github.io/adaptive-skills/skills/feature-value-governance/) | Product | Deciding whether a feature deserves investment |
| [`opportunity-tree-alignment`](https://nevitonsantana.github.io/adaptive-skills/skills/opportunity-tree-alignment/) | Product | Backlog bets lack clear outcome links |
| [`business-design`](https://nevitonsantana.github.io/adaptive-skills/skills/business-design/) | Business | Framing product or business strategy from mixed signals |
| [`revenue-lever-mapping`](https://nevitonsantana.github.io/adaptive-skills/skills/revenue-lever-mapping/) | Business | A value claim has no measurable mechanism |
| [`feature-complexity-audit`](https://nevitonsantana.github.io/adaptive-skills/skills/feature-complexity-audit/) | Governance | Permanent feature cost may change a build decision |
| [`sunset-decision`](https://nevitonsantana.github.io/adaptive-skills/skills/sunset-decision/) | Governance | An existing feature may no longer justify its cost |
| [`knowledge-source-evaluation`](https://nevitonsantana.github.io/adaptive-skills/skills/knowledge-source-evaluation/) | Governance | A document may become governed knowledge |
| [`knowledge-conflict-resolution`](https://nevitonsantana.github.io/adaptive-skills/skills/knowledge-conflict-resolution/) | Governance | Decision-relevant sources disagree |
| [`restricted-context-check`](https://nevitonsantana.github.io/adaptive-skills/skills/restricted-context-check/) | Governance | Sensitive or untrusted knowledge may enter task context |
| [`qa-review`](https://nevitonsantana.github.io/adaptive-skills/skills/qa-review/) | Quality | A change needs a cross-layer quality gate |
| [`observability-review`](https://nevitonsantana.github.io/adaptive-skills/skills/observability-review/) | Metrics | Metrics, events, alerts, or diagnostics need review |

The [skills catalog](https://nevitonsantana.github.io/adaptive-skills/getting-started/skill-catalog/) provides the same complete inventory with expected outcomes for faster selection.

## What each profile includes

- the canonical description, category, version, and owner
- direct navigation to usage, restrictions, core moves, outputs, and verification
- the complete skill contract
- an explicit link to the source file

Public profiles are a reader-facing projection. They do not change skill behavior, routing authority, or runtime execution.
